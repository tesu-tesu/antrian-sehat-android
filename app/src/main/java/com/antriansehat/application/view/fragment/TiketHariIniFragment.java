...
import com.antriansehat.application.databinding.FragmentTiketHariIniBinding;
...
public class TiketHariIniFragment extends Fragment implements RiwayatTiketContract.View {
    private FragmentTiketHariIniBinding binding;
    private RiwayatTiketContract.Presenter presenter;

    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = FragmentTiketHariIniBinding.inflate(getLayoutInflater());
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        presenter.requestTicket(RiwayatTiketContract.TODAY_TICKET);
    ...
    public void showWaitingList(List<WaitingList> waitingLists) {
        binding.rvWaitingList.setAdapter(new RiwayatTiketHariIniAdapter(waitingLists, getLayoutInflater()));

        if(waitingLists.size() == 0) {
            binding.tvNoTicket.setVisibility(View.VISIBLE);
            return;
        }
        ((RiwayatTiketHariIniAdapter) binding.rvWaitingList.getAdapter()).setOnItemClickListener(new RiwayatTiketHariIniAdapter.ItemClickListener() {
            @Override
